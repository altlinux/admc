...
: QSortFilterProxyModel(parent)
{
    ...
    const BoolSettingSignal *dn_column_setting = Settings::instance()->get_bool_signal(BoolSetting_DnColumn);
    connect(
        dn_column_setting, &BoolSettingSignal::changed,
        this, &DnColumnProxy::on_toggle_show_dn_column);
    on_toggle_show_dn_column();
}

void DnColumnProxy::on_toggle_show_dn_column() {
    show_dn_column = Settings::instance()->get_bool(BoolSetting_DnColumn);

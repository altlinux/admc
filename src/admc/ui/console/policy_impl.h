...
public:
    ...
    bool can_drop(const QList<QPersistentModelIndex> &dropped_list,
                  const QSet<int> &dropped_type_list,
                  const QPersistentModelIndex &target,
                  const int target_type) override;
    void drop(const QList<QPersistentModelIndex> &dropped_list,
              const QSet<int> &dropped_type_list,
              const QPersistentModelIndex &target,
              const int target_type) override;
    ...
    QSet<QAction *> get_custom_actions(
        ...
        const bool single_selection) const override;
...
private slots:
    void on_add_link();
    void on_edit();
    void on_ou_gplink_changed(const QString &ou_dn,
                              const Gplink &gplink,
...
private:
    ...
    void set_policy_item_icon(const QModelIndex &policy_index,
                              bool is_checked,
                              GplinkOption option);
...
void set_policy_link_icon(QStandardItem *policy_item,
                          bool is_enforced,
                          bool is_disabled);
void set_enforced_policy_icon(QStandardItem *policy_item,
                              bool is_enforced);

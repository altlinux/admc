...
: ConsoleImpl(console_arg) {
    ...
    connect(
        enforce_action, &QAction::triggered, [this](){
            on_change_gplink_option_action(enforce_action);});
    connect(
        disable_action, &QAction::triggered, [this](){
            on_change_gplink_option_action(disable_action);});
}

bool PolicyImpl::can_drop(const QList<QPersistentModelIndex> &dropped_list,
    ...
    const bool dropped_are_policy_ou =
        (dropped_type_list == QSet<int>({ItemType_PolicyOU}));

    return dropped_are_policy_ou;
}

void PolicyImpl::drop(const QList<QPersistentModelIndex> &dropped_list,
                      const QSet<int> &dropped_type_list,
                      const QPersistentModelIndex &target,
                      const int target_type) {
    Q_UNUSED(target_type);
    ...
    const QList<QModelIndex> dropped_list_normal =
        normal_index_list(dropped_list);
    const QList<QString> ou_list =
        index_list_to_dn_list(dropped_list_normal, PolicyOURole_DN);
...
void PolicyImpl::selected_as_scope(const QModelIndex &index) {
    ...
    // Dont check perms if item was already fetched. It helps to avoid excessive
    // freezes.
    if (!was_fetched) {
    ...
    if (!perms_ok && ok) {
        ...
        const QString text =
            tr("Permissions for this policy's GPT don't match the permissions for it's GPC object. Would you like to update GPT permissions?");

        auto sync_warning_dialog = new QMessageBox(console);
        sync_warning_dialog->setAttribute(Qt::WA_DeleteOnClose);
        sync_warning_dialog->setStandardButtons(
...
QSet<QAction *> PolicyImpl::get_custom_actions(
...
QSet<StandardAction> PolicyImpl::get_standard_actions(
    const QModelIndex &index,
    const bool single_selection) const
{
    Q_UNUSED(index);
...
void PolicyImpl::delete_action(const QList<QModelIndex> &index_list) {
    ...
    if (parent_is_ou) {
        ...
        console_policy_remove_link({ console },
                                   policy_results,
                                   ItemType_Policy,
                                   PolicyRole_DN,
                                   ou_dn);
    } else {
        console_policy_delete({ console },
                              policy_results,
                              ItemType_Policy,
                              PolicyRole_DN);
    }
...
void PolicyImpl::properties(const QList<QModelIndex> &index_list) {
    ...
    console_policy_properties({ console },
                              policy_results,
                              ItemType_Policy,
                              PolicyRole_DN);
}
...
void PolicyImpl::on_add_link() {
    console_policy_add_link({ console },
                            policy_results,
                            ItemType_Policy,
                            PolicyRole_DN);
}
...
void PolicyImpl::on_ou_gplink_changed(const QString &ou_dn,
                                      const Gplink &gplink,
                                      const QString &policy_dn,
                                      GplinkOption option) {
    ...
    QModelIndex target_policy_index =
        get_ou_child_policy_index(console, ou_item_index, policy_dn);
    ...
    if (option != GplinkOption_NoOption) {
        set_policy_item_icon(target_policy_index,
                             gplink.get_option(policy_dn, option),
                             option);
    }
}
...
void PolicyImpl::on_change_gplink_option_action(QAction *action)
{
    ...
    bool success = ad.attribute_replace_string(ou_dn,
                                               ATTRIBUTE_GPLINK,
                                               updated_gplink_string);
    if (success) {
...
{
    const QModelIndex policy_index = console->get_current_scope_item();
    if (policy_index.parent().data(ConsoleRole_Type).toInt()
        != ItemType_PolicyOU)
        return;
...
void PolicyImpl::set_policy_item_icon(const QModelIndex &policy_index,
                                      bool is_checked,
                                      GplinkOption option)
{
    QStandardItem *target_policy_item = console->get_item(policy_index);
    if (target_policy_item->parent()->data(ConsoleRole_Type).toInt()
        != ItemType_PolicyOU)
        return;
...
void set_policy_link_icon(QStandardItem *policy_item,
                          bool is_enforced,
                          bool is_disabled) {
    if (is_enforced) {
        if (!is_disabled)
            policy_item->setIcon(
                g_icon_manager->item_icon(ItemIcon_Policy_Enforced));
        else
            policy_item->setIcon(
                g_icon_manager->item_icon(ItemIcon_Policy_Enforced_Disabled));
    } else {
        if (!is_disabled)
            policy_item->setIcon(
                g_icon_manager->item_icon(ItemIcon_Policy_Link));
        else
            policy_item->setIcon(
                g_icon_manager->item_icon(ItemIcon_Policy_Link_Disabled));
    }
}
...
void console_policy_load_item(QStandardItem *main_item,
    ...
    const QString gpo_status =
        gpo_status_from_int(object.get_int(ATTRIBUTE_FLAGS));
    main_item->setData(gpo_status, PolicyRole_GPO_Status);
}

void console_policy_edit(ConsoleWidget *console,
                         const int item_type,
                         const int dn_role) {
...
void console_policy_rename(const QList<ConsoleWidget *> &console_list,
    ...
    const QString dn = get_selected_target_dn(console_list[0],
                                              item_type,
                                              dn_role);
    ...
    QObject::connect(
        ...
        [console_list, policy_results, dn]() {
            ...
            // NOTE: ok to not update dn after rename because policy rename
            // doesn't change dn, since "policy name" is displayName
            auto apply_changes =
                [&dn, &object, policy_results](ConsoleWidget *target_console) {
                    ...
                    if (policy_root.isValid()) {
                        const QList<QModelIndex> index_list =
                            target_console->search_items(policy_root,
                                                         PolicyRole_DN,
                                                         dn,
                                                         { ItemType_Policy });
                    ...
                    const QModelIndex find_policy_root =
                        get_find_policy_root(target_console);

                    if (find_policy_root.isValid()) {
                        const QModelIndex index =
                            target_console->search_item(
                                find_policy_root,
                                FoundPolicyRole_DN,
                                dn,
                    ...
                    console_policy_update_policy_results(target_console,
                                                         policy_results);
                };
...
void console_policy_add_link(const QList<ConsoleWidget *> &console_list,
                             PolicyResultsWidget *policy_results,
                             const int item_type,
                             const int dn_role) {
    auto dialog = new SelectObjectDialog({ CLASS_OU },
                                         SelectObjectDialogMultiSelection_Yes,
                                         console_list[0]);
    dialog->setWindowTitle(QCoreApplication::translate("PolicyImpl",
                                                       "Add Link"));
    dialog->open();
    ...
    QObject::connect(
        ...
        [console_list, policy_results, dialog, item_type, dn_role]() {
            const QList<QString> gpo_list =
                get_selected_dn_list(console_list[0],
                                     item_type,
                                     dn_role);
...
void console_policy_remove_link(const QList<ConsoleWidget *> &console_list,
                                ...
                                const int dn_role,
                                const QString &ou_dn) {
    const QList<QString> dn_list = get_selected_dn_list(console_list[0],
                                                        item_type,
                                                        dn_role);
    ...
    const QString confirmation_text =
        QCoreApplication::translate(
            "PolicyImpl",
            "Are you sure you want to unlink this policy from the OU? Note that the actual policy object won't be deleted.");
    const bool confirmed = confirmation_dialog(confirmation_text,
                                               console_list[0]);
    if (!confirmed) {
        return;
    }
    ...
    const QString gplink_old_string =
        parent_object.get_string(ATTRIBUTE_GPLINK);
    ...
    const bool replace_success = ad.attribute_replace_string(ou_dn,
                                                             ATTRIBUTE_GPLINK,
                                                             gplink_new_string);
    ...
    if (replace_success) {
        auto apply_changes =
            [&ou_dn, &dn_list,
             &gplink_new_string,
             policy_results](ConsoleWidget *target_console) {
                const QModelIndex policy_root =
                    get_policy_tree_root(target_console);
                ...
                // displayed under multiple OU's
                if (policy_root.isValid()) {
                    const QModelIndex ou_index =
                        target_console->search_item(policy_root,
                                                    PolicyOURole_DN,
                                                    ou_dn,
                                                    { ItemType_PolicyOU });
                    ...
                    if (ou_index.isValid()) {
                        ...
                        for (const QString &dn : dn_list) {
                            const QModelIndex gpo_index =
                                get_ou_child_policy_index(target_console,
                                                          ou_index,
                                                          dn);
...
void console_policy_delete(const QList<ConsoleWidget *> &console_list,
                           PolicyResultsWidget *policy_results,
                           const int item_type,
                           const int dn_role) {
    AdInterface ad;
    ...
    if ((! fsmo_is_current_dc_master_for_role(ad, FSMORole_PDCEmulation))
        && gpo_edit_without_PDC_disabled) {
        QMessageBox::StandardButton answer =
            QMessageBox::question(
                console_list[0],
                QObject::tr("Deletion is not available"),
    ...
    QList<QString> dn_list = get_selected_dn_list(console_list[0],
                                                  item_type,
                                                  dn_role);
    ...
    const QString confirmation_text = (dn_list.size() > 1) ?
        QCoreApplication::translate(
            "PolicyImpl",
            "Are you sure you want to delete these policies and all their links?") :
        QCoreApplication::translate(
            "PolicyImpl",
            "Are you sure you want to delete this policy and all of it's links?");
    const bool confirmed =
        confirmation_dialog(confirmation_text, console_list[0]);
    if (!confirmed) {
        return;
    }
    ...
    auto apply_changes =
        [&deleted_list, policy_results](ConsoleWidget *target_console) {
            ...
            if (find_policy_root.isValid()) {
                for (const QString &dn : deleted_list) {
                    const QList<QModelIndex> index_list =
                        target_console->search_items(find_policy_root,
                                                     FoundPolicyRole_DN,
                                                     dn,
                                                     { ItemType_FoundPolicy });
                    const QList<QPersistentModelIndex> persistent_list =
...
void console_policy_properties(const QList<ConsoleWidget *> &console_list,
                               PolicyResultsWidget *policy_results,
                               const int item_type,
                               const int dn_role) {
    AdInterface ad;
    ...
    PropertiesDialog *dialog =
        PropertiesDialog::open_for_target(ad, dn, &dialog_is_new);
    ...
    auto on_propeties_applied = [console_list, policy_results, dn]() {
        ...
        auto apply_changes =
            [policy_results, &dn, &object](ConsoleWidget *target_console) {
                const QModelIndex policy_root =
                    get_policy_tree_root(target_console);
...
void policy_add_links(const QList<ConsoleWidget *> &console_list,
    ...
    auto apply_changes =
        [&ou_list, &gpo_object_list](ConsoleWidget *target_console) {
            const QModelIndex policy_root =
                get_policy_tree_root(target_console);
            ...
            if (policy_root.isValid()) {
                for (const QString &ou_dn : ou_list) {
                    ...
                    const bool ou_was_fetched =
                        console_item_get_was_fetched(ou_index);

                    if (! ou_was_fetched) {
                        continue;
                    }

                    policy_ou_impl_add_objects_to_console(target_console,
                                                          gpo_object_list,
                                                          ou_index);
...
void console_policy_update_policy_results(ConsoleWidget *console,
    ...
    const QString scope_gpo = get_selected_target_dn(console,
                                                     ItemType_Policy,
                                                     PolicyRole_DN);
...
bool policy_is_enforced(QStandardItem *policy_item)
{
    ...
    const QString ou_dn =
        policy_item->parent()->data(PolicyOURole_DN).toString();
    const QString gplink_string = g_gplink_manager->ou_gplink(ou_dn);
    const Gplink gplink = Gplink(gplink_string);
    is_enforced =
        gplink.enforced_gpo_dn_list().contains(
            policy_item->data(PolicyRole_DN).toString());
...
bool policy_is_disabled(QStandardItem *policy_item)
{
    ...
    const Gplink gplink = Gplink(gplink_string);
    is_disabled =
        gplink.disabled_gpo_dn_list().contains(
            policy_item->data(PolicyRole_DN).toString());
...
void console_policy_edit(const QString &policy_dn, ConsoleWidget *console) {
    ...
    if ((! fsmo_is_current_dc_master_for_role(ad, FSMORole_PDCEmulation))
        ...
        QMessageBox::StandardButton answer =
            QMessageBox::question(
                console,
                QObject::tr("Edition is not available"),
                QObject::tr("ADMC is connected to DC without the PDC-Emulator role - "
    ...
    auto on_gpui_error = [console](QProcess::ProcessError error) {
        ...
        if (failed_to_start) {
            const QString error_text =
                QObject::tr("Failed to start GPUI. Check that it's installed.");
            qDebug() << error_text;

    ...
    parent_dn(parent_dn_arg)
{
    ...
    if (type_arg == SitesLinkType::Link) {
        ui->clue_label->setText(
            tr("Site link object must link two or more sites"));
        ...
        ui->clue_label->setText(
            tr("Link bridge object must link two or more site links"));
...
void CreateSitesLinkDialog::accept() {
    ...
    const int count = ui->sites_link_common_wget->right_list_wget()->count();
    const bool is_link_type = (type == SitesLinkType::Link);
    if (count < min_linked_objects_count) {
        ...
        message_box_warning(
            ui->sites_link_common_wget, tr("Error"), warning_text);
        return;
    }
    ...
    const QString obj_class =
        is_link_type ? CLASS_SITE_LINK : CLASS_SITE_LINK_BRIDGE;
    ...
    auto right_list_wget = ui->sites_link_common_wget->right_list_wget();
    const int wget_count = right_list_wget->count();
    for (int i = 0; i < wget_count; ++i) {
        auto linked_dn =
    ...
    QString message;
    if (! add_success) {
        message = is_link_type ?
            tr("Failed to create site link object %1").arg(name) :
    ...
    created_dn = dn;
    message = type == SitesLinkType::Link ?
        tr("Site link object %1 has been successfully created.").arg(name) :

...
void LoginDialog::show() {
    ...
    const QString site = Settings::instance()->get_value(SettingsValue_Site).toString();
...
void LoginDialog::complete(const QString &host) {
    ...
    if (logged_in) {
        ...
        if (save_session) {
            const QString site = site_edit->text();
            Settings::instance()->set_value(SettingsValue_Domain, domain);
            Settings::instance()->set_value(SettingsValue_Site, site);
            Settings::instance()->set_value(SettingsValue_Host, host);
        }

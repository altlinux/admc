 ...
 * Copyright (C) 2024 DimDone
 * Copyright (C) 2025 august-alt
...
static QString tr(const char* str) {
    static const char* CONTEXT = "AdConfig";
...
QString AdConfig::pso_container_dn() const {
    return QString("CN=Password Settings Container,CN=System,%1")
        .arg(domain_dn());
}
...
const QHash<Attribute, QString> AdConfig::get_fallback_display_names() const {
    return {
        ...
        {ATTRIBUTE_SAM_ACCOUNT_NAME, tr("Logon name (pre-Windows 2000)")},
        {ATTRIBUTE_MAIL, tr("E-mail")},
        {ATTRIBUTE_LOCATION, tr("Location")},
        {ATTRIBUTE_MANAGED_BY,
         QCoreApplication::translate("managedBy", "Managed by")},
    };
}
...
QList<QString> AdConfig::get_possible_superiors(
    const QList<ObjectClass> &object_classes) const
{
    QList<QString> out;
...
QList<QString> AdConfig::get_optional_attributes(
    const QList<QString> &object_classes) const
{
...
QList<QString> AdConfig::get_mandatory_attributes(
    const QList<QString> &object_classes) const
{
    const QList<QString> all_classes = d->add_auxiliary_classes(object_classes);
    QList<QString> attributes;
...
AttributeType AdConfig::get_attribute_type(const QString &attribute) const {
    ...
    const QString attribute_syntax =
        schema.get_string(ATTRIBUTE_ATTRIBUTE_SYNTAX);
    const QString om_syntax = schema.get_string(ATTRIBUTE_OM_SYNTAX);
    if (type_map.contains(attribute_syntax) &&
        type_map[attribute_syntax].contains(om_syntax)) {
...
LargeIntegerSubtype AdConfig::get_attribute_large_integer_subtype(
...
bool AdConfig::get_attribute_is_backlink(const QString &attribute) const {
    if (d->attribute_schemas[attribute].contains(ATTRIBUTE_LINK_ID)) {
        const int link_id =
            d->attribute_schemas[attribute].get_int(ATTRIBUTE_LINK_ID);
...
bool AdConfig::get_attribute_is_constructed(const QString &attribute) const {
    const int system_flags =
        d->attribute_schemas[attribute].get_int(ATTRIBUTE_SYSTEM_FLAGS);
...
// NOTE: technically, Active Directory provides translations for right names but
// it's not accessible, so have to translate these ourselves. On Windows, you
// would use the localizationDisplayId retrieved from schema to get translation
// from dssec.dll. And we don't have dssec.dll, nor do we have the ability to
// interact with it!
QString AdConfig::get_right_name(const QByteArray &right_guid,
                                 const QLocale::Language language) const {
    const QHash<QString, QString> cn_to_map_russian = {
        {"DS-Replication-Get-Changes", tr("DS Replication Get Changes")},
        {"DS-Replication-Get-Changes-All", tr("DS Replication Get Changes All")},
        {"Email-Information", tr("Phone and Mail Options")},
        {"DS-Bypass-Quota", tr("Bypass the quota restrictions during creation.")},
        {"Receive-As", tr("Receive As")},
        {"Unexpire-Password", tr("Unexpire Password")},
        {"Do-Garbage-Collection", tr("Do Garbage Collection")},
        {"Allowed-To-Authenticate", tr("Allowed To Authenticate")},
        {"Change-PDC", tr("Change PDC")},
        {"Reanimate-Tombstones", tr("Reanimate Tombstones")},
        {"msmq-Peek-Dead-Letter", tr("msmq Peek Dead Letter")},
        {"Certificate-AutoEnrollment", tr("AutoEnrollment")},
        {"DS-Install-Replica", tr("DS Install Replica")},
        {"Domain-Password", tr("Domain Password & Lockout Policies")},
        {"Generate-RSoP-Logging", tr("Generate RSoP Logging")},
        {"Run-Protect-Admin-Groups-Task", tr("Run Protect Admin Groups Task")},
        {"Self-Membership", tr("Self Membership")},
        {"DS-Clone-Domain-Controller", tr("Allow a DC to create a clone of itself")},
        {"Domain-Other-Parameters", tr("Other Domain Parameters (for use by SAM)")},
        {"SAM-Enumerate-Entire-Domain", tr("SAM Enumerate Entire Domain")},
        {"DS-Write-Partition-Secrets", tr("Write secret attributes of objects in a Partition")},
        {"Send-As", tr("Send As")},
        {"DS-Replication-Manage-Topology", tr("DS Replication Manage Topology")},
        {"DS-Set-Owner", tr("Set Owner of an object during creation.")},
        {"Generate-RSoP-Planning", tr("Generate RSoP Planning")},
        {"Certificate-Enrollment", tr("Certificate Enrollment")},
        {"Web-Information", tr("Web Information")},
        {"Create-Inbound-Forest-Trust", tr("Create Inbound Forest Trust")},
        {"Migrate-SID-History", tr("Migrate SID History")},
        {"Update-Password-Not-Required-Bit", tr("Update Password Not Required Bit")},
        {"MS-TS-GatewayAccess", tr("MS-TS-GatewayAccess")},
        {"Validated-MS-DS-Additional-DNS-Host-Name", tr("Validated write to MS DS Additional DNS Host Name")},
        {"msmq-Receive", tr("msmq Receive")},
        {"Validated-DNS-Host-Name", tr("Validated DNS Host Name")},
        {"Send-To", tr("Send To")},
        {"DS-Replication-Get-Changes-In-Filtered-Set", tr("DS Replication Get Changes In Filtered Set")},
        {"Read-Only-Replication-Secret-Synchronization", tr("Read Only Replication Secret Synchronization")},
        {"Validated-MS-DS-Behavior-Version", tr("Validated write to MS DS behavior version")},
        {"msmq-Open-Connector", tr("msmq Open Connector")},
        {"Terminal-Server-License-Server", tr("Terminal Server License Server")},
        {"Change-Schema-Master", tr("Change Schema Master")},
        {"Recalculate-Hierarchy", tr("Recalculate Hierarchy")},
        {"DS-Check-Stale-Phantoms", tr("DS Check Stale Phantoms")},
        {"msmq-Receive-computer-Journal", tr("msmq Receive computer Journal")},
        {"User-Force-Change-Password", tr("User Force Change Password")},
        {"Domain-Administer-Server", tr("Domain Administer Server")},
        {"DS-Replication-Synchronize", tr("DS Replication Synchronize")},
        {"Personal-Information", tr("Personal Information")},
        {"msmq-Peek", tr("msmq Peek")},
        {"General-Information", tr("General Information")},
        {"Membership", tr("Group Membership")},
        {"Add-GUID", tr("Add GUID")},
        {"RAS-Information", tr("Remote Access Information")},
        {"DS-Execute-Intentions-Script", tr("DS Execute Intentions Script")},
        {"Allocate-Rids", tr("Allocate Rids")},
        {"Update-Schema-Cache", tr("Update Schema Cache")},
        {"Apply-Group-Policy", tr("Apply Group Policy")},
        {"User-Account-Restrictions", tr("Account Restrictions")},
        {"Validated-SPN", tr("Validated SPN")},
        {"DS-Read-Partition-Secrets", tr("Read secret attributes of objects in a Partition")},
        {"User-Logon", tr("Logon Information")},
        {"DS-Query-Self-Quota", tr("DS Query Self Quota")},
        {"Change-Infrastructure-Master", tr("Change Infrastructure Master")},
        {"Open-Address-Book", tr("Open Address Book")},
        {"User-Change-Password", tr("User Change Password")},
        {"msmq-Peek-computer-Journal", tr("msmq Peek computer Journal")},
        {"Change-Domain-Master", tr("Change Domain Master")},
        {"msmq-Send", tr("msmq Send")},
        {"Change-Rid-Master", tr("Change Rid Master")},
        {"Recalculate-Security-Inheritance", tr("Recalculate Security Inheritance")},
        {"Refresh-Group-Cache", tr("Refresh Group Cache")},
        {"Manage-Optional-Features", tr("Manage Optional Features")},
        {"Reload-SSL-Certificate", tr("Reload SSL Certificate")},
        {"Enable-Per-User-Reversibly-Encrypted-Password", tr("Enable Per User Reversibly Encrypted Password")},
        {"DS-Replication-Monitor-Topology", tr("DS Replication Monitor Topology")},
        {"Public-Information", tr("Public Information")},
        {"Private-Information", tr("Private Information")},
        {"msmq-Receive-Dead-Letter", tr("msmq Receive Dead Letter")},
        {"msmq-Receive-journal", tr("msmq Receive journal")},
        {"DNS-Host-Name-Attributes", tr("DNS Host Name Attributes")},
    };
    ...
    const QString out = d->rights_guid_to_name_map.value(
        right_guid,
        tr("<unknown rights>"));
    return out;
}
...
bool AdConfig::rights_applies_to_class(const QString &rights_cn,
                                       const QList<QString> &class_list) const {
    ...
    const QSet<QString> applies_to_set = QSet<QString>(applies_to_list.begin(),
                                                       applies_to_list.end());

    const QSet<QString> class_set = QSet<QString>(class_list.begin(),
                                                  class_list.end());
...
QStringList AdConfig::get_permissionable_attributes(
    const QString &obj_class) const
{
...
bool AdConfig::class_is_auxiliary(const QString &obj_class) const {
    ...
    const int class_category =
        d->class_schemas[obj_class].get_int(ATTRIBUTE_OBJECT_CLASS_CATEGORY);
...
const QList<QString> AdConfig::get_applies_to_classes(
    const AdObject &object) const
{
    QList<QString> out;

    const QList<QString> class_guid_list =
        object.get_strings(ATTRIBUTE_APPLIES_TO);
...
void AdConfig::load_extended_rights(AdInterface &ad)
{
    ...
    const QHash<QString, AdObject> search_results =
        ad.search(search_base, SearchScope_Children, filter, attributes);
...
void AdConfig::load_attribute_schemas(AdInterface &ad) {
    ...
    const QHash<QString, AdObject> results =
        ad.search(schema_dn(), SearchScope_Children, filter, attributes);
...
void AdConfig::load_class_schemas(AdInterface &ad) {
    ...
    for (const AdObject &object : results.values()) {
        const QString object_class =
            object.get_string(ATTRIBUTE_LDAP_DISPLAY_NAME);
        ...
        const QStringList possible_inferiors =
            bytearray_list_to_string_list(
                object.get_values(ATTRIBUTE_POSSIBLE_INFERIORS));
...
void AdConfig::load_display_names(AdInterface &ad, const QString &locale_dir) {
    ...
    const QHash<QString, AdObject> results = ad.search(locale_dir,
    ...
    for (const AdObject &object : results) {
        ...
        if (object.contains(ATTRIBUTE_CLASS_DISPLAY_NAME)) {
            d->class_display_names[object_class] =
                object.get_string(ATTRIBUTE_CLASS_DISPLAY_NAME);
        }

        if (object.contains(ATTRIBUTE_ATTRIBUTE_DISPLAY_NAMES)) {
            const QList<QString> display_names =
                object.get_strings(ATTRIBUTE_ATTRIBUTE_DISPLAY_NAMES);
            ...
            for (const auto &display_name_pair : display_names) {
                ...
                d->attribute_display_names[object_class][attribute_name] =
                    display_name;
            }
...
const QList<QString> get_categories(const AdObject &object) {
    QList<QString> categories_out =
        object.get_strings(ATTRIBUTE_FILTER_CONTAINERS);
...
void AdConfig::load_filter_containers(AdInterface &ad,
                                      const QString &locale_dir) {
    const QString ui_settings_dn = QString("CN=DS-UI-Default-Settings,%1")
        .arg(locale_dir);
    const AdObject object = ad.search_object(ui_settings_dn,
                                             { ATTRIBUTE_FILTER_CONTAINERS });
    ...
    for (const auto &object_category : categories) {
        const QString category_dn =
            QString("CN=%1,%2").arg(object_category, schema_dn());
        const AdObject category_object = ad.search_object(
            category_dn, { ATTRIBUTE_LDAP_DISPLAY_NAME });
        const QString object_class =
            category_object.get_string(ATTRIBUTE_LDAP_DISPLAY_NAME);
    ...
    // Make configuration and schema pass filter in dev mode so they are visible
    // and can be fetched
...
void AdConfig::load_permissionable_attributes(const QString &obj_class,
    ...
    QHash<QString, AdObject> results = ad.search(
        schema_dn(), SearchScope_Children, filter,
    ...
    const QStringList allowed_attrs =
        bytearray_list_to_string_list(
            class_schema_object.get_values(ATTRIBUTE_ALLOWED_ATTRIBUTES));
    const QStringList attrs_system_may_contain =
        bytearray_list_to_string_list(
            class_schema_object.get_values(ATTRIBUTE_SYSTEM_MAY_CONTAIN));

    // Use set to remove duplicates (just in case)
    QSet<QString> allowed_attrs_set = QSet<QString>(allowed_attrs.begin(),
                                                    allowed_attrs.end());
    QSet<QString> may_contain_attrs_set =
        QSet<QString>(attrs_system_may_contain.begin(),
                      attrs_system_may_contain.end());
    ...
    for (const QString &attr : allowed_attrs_set) {
        if (get_attribute_is_backlink(attr) ||
            get_attribute_is_constructed(attr) ||
            get_attribute_is_system_only(attr)) {
            permissionable_attrs_set.remove(attr);
        }
    }

    QStringList permissionable_attrs =
        QStringList(permissionable_attrs_set.begin(),
                    permissionable_attrs_set.end());
...
QList<QString> AdConfigPrivate::add_auxiliary_classes(
    const QList<QString> &object_classes) const
{
    QList<QString> out;
